...
##实现了从t_start计算到t_end时间步的累积产量和净现值,用于最后的后处理计算，被calculate_cum_prod.py调用
import numpy as np
import os
def profit_calcualtion_1(t_start,t_end,time_step,area_well,delta_t,gas_price,filename_v1,current_directory,pressure1):
    '''
    这里只传入gas_price，而gas_cost由传入的pressure数组在内部计算，因为要预测time_step个时间步，gas_cost是一个数组
    '''
    gas_file='cumulative_production'
...
#    q_sum_1=(sum(v_sum_well_1))*area_well*delta_t
    #把每个时间段的每口井的产量输出换成每个时间段的每口井的净现值输出
        
    gas_cost=(101325-pressure1)*2.0e-10  ##pressure应该是一个shape为（t_end-t_start,）的array，这样才能跟v_sum_well_1匹配
    profit_1=v_sum_well_1*gas_price*area_well*delta_t-gas_cost
    return profit_1

def profit_calcualtion_2(t_start,t_end,time_step,area_well,delta_t,gas_price,filename_v2,current_directory,pressure2): 
    gas_file='cumulative_production'
...
#    args_path_2=r'E:\EnRML_Gas_Modelling\water_gas\water_gas_line_3_19\gas_{0}\water_gas_ply_PLY_3_t2.tec'.format(i)   
    ...
    gas_cost=(101325-pressure2)*2.0e-10  ##pressure应该是一个shape为（t_end-t_start,）的array，这样才能跟v_sum_well_1匹配
    profit_2=v_sum_well_2*gas_price*area_well*delta_t-gas_cost
    return profit_2

def profit_calcualtion_3(t_start,t_end,time_step,area_well,delta_t,gas_price,filename_v3,current_directory,pressure3): 
    gas_file='cumulative_production'
...
#    args_path_3=r'E:\EnRML_Gas_Modelling\water_gas\water_gas_line_3_19\gas_{0}\water_gas_ply_PLY_4_t3.tec'.format(i)  
    ...
    gas_cost=(101325-pressure3)*2.0e-10  ##pressure应该是一个shape为（t_end-t_start,）的array，这样才能跟v_sum_well_1匹配
    profit_3=v_sum_well_3*gas_price*area_well*delta_t-gas_cost
    return profit_3
...
#        print content_1
        ...
        v_sum_1=sum(v_1)
        v_sum_well_1[x-t_start]=v_sum_1
    pressure3=np.array([x_ini[0]]*20)
    gas_cost=(101325-pressure3)*2.0e-10  ##pressure应该是一个shape为（t_end-t_start,）的array，这样才能跟v_sum_well_1匹配
    profit_3=v_sum_well_1*gas_price-gas_cost
